...
from models import SwinUNETR_Multi_Decoder_Fusion
from metrics import dice_coefficient_batch, compute_acc_batch, compute_f1_batch
import optimizers
import data

class MultiDecoderFusionSegtrainer(pl.LightningModule):
    def __init__(
        ...
        if model_name == 'swin_unetr_multi_decoder_fusion':
            self.model = SwinUNETR_Multi_Decoder_Fusion(**model_dict)
        else:
    ...
    def forward_test(self, x):
        output, fusion_output = self.model(x)
    ...
    def training_step(self, batch, batch_idx):
        ...
        output, fusion_output = self.forward(images)
        loss = 0
        for i in range(self.model.num_decoder):
            loss += self.loss_function(output[i], mix_labels[:, i, :, :, :].unsqueeze(1))
        loss += self.loss_function(fusion_output, mix_labels[:, 0, :, :, :].unsqueeze(1))
        # logging
        self.log(
            "train/bce_loss_step",
            loss,
