...
__all__ = [
    "SwinUNETR_Multi_Decoder_Fusion"
]


class SwinUNETR_Multi_Decoder_Fusion(nn.Module):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            num_decoder: number of decode branch for multi scale fitting target
        ...
        self.decoder5_lst = nn.ModuleList()
        self.decoder4_lst = nn.ModuleList()
        self.decoder3_lst = nn.ModuleList()
        self.decoder2_lst = nn.ModuleList()
        self.decoder1_lst = nn.ModuleList()
        self.out_lst = nn.ModuleList()
        self.num_decoder =num_decoder
        for k in range(self.num_decoder):
            self.decoder5_lst.append(
                UnetrUpBlock(
                spatial_dims=spatial_dims,
                in_channels=16 * feature_size,
    ...
    def forward(self, x_in):
        ...
        output = []
        for k in range(self.num_decoder):
            dec3 = self.decoder5_lst[k](dec4, hidden_states_out[3])
            dec2 = self.decoder4_lst[k](dec3, enc3)
            dec1 = self.decoder3_lst[k](dec2, enc2)
            dec0 = self.decoder2_lst[k](dec1, enc1)
